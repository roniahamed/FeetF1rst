...
def create_email_verification(sender, instance, created, **kwargs):
    if created and not instance.is_verified:
        ...
        html_message = render_to_string('email/email_verification.html', {'user': instance, 'otp': code, 'full_name': instance.full_name})

        OTP.objects.create(user=instance, code=code, purpose='email_verification')

        send_verification_email(subject = subject, user=instance, email = instance.email, otp=code, html_template=html_message)